...
export function TitleBar() {
  ...
  const { repos, skills, composedPrompt, activeWorkspace } = useAppStore(
    useShallow((s) => ({
      repos: s.repos,
      skills: s.skills,
  ...
  const [projectName, setProjectName] = useState(
    activeWorkspace?.name ?? "Untitled Project",
  );
  ...
  return (
    ...
    >
      {/* Left: Branding + Editable Project Name */}
      <div data-tauri-drag-region className="flex items-center gap-3 min-w-0">
        ...
        <input
          ...
          className="text-[11px] text-v-text bg-transparent border-none outline-none w-[160px] truncate hover:bg-v-surface/50 focus:bg-v-surface px-1.5 py-0.5 rounded transition-colors"
          title="Project name"
        />
      </div>
