...
describe("agentSlice", () => {
  ...
  describe("session lifecycle", () => {
    it("creates a session and auto-activates it", () => {
      store.getState().createSessionLocal("s1", "Session 1");
      const state = store.getState();
      expect(state.agentSessions.has("s1")).toBe(true);
      expect(state.activeSessionId).toBe("s1");
      expect(state.agentSessions.get("s1")!.name).toBe("Session 1");
    });

    it("second session does not override active", () => {
      store.getState().createSessionLocal("s1", "Session 1");
      store.getState().createSessionLocal("s2", "Session 2");
    ...
    it("switches active session", () => {
      store.getState().createSessionLocal("s1", "Session 1");
      store.getState().createSessionLocal("s2", "Session 2");
      store.getState().setActiveSessionId("s2");
    ...
    it("removes session and reassigns active", () => {
      ...
      expect(store.getState().agentSessions.has("s1")).toBe(false);
      expect(store.getState().activeSessionId).toBe("s2");
    });
  ...
  describe("chat messages", () => {
    ...
    it("adds a user message to session", () => {
      ...
      const session = store.getState().agentSessions.get("s1")!;
    ...
    it("does nothing for non-existent session", () => {
      ...
      expect(store.getState().agentSessions.get("s1")!.chatMessages).toHaveLength(0);
    });
  ...
  describe("no duplication", () => {
    ...
    it("simulates the fixed hook behavior: only session-scoped call when sid exists", () => {
      ...
      const session = store.getState().agentSessions.get(sid)!;
      expect(session.chatMessages).toHaveLength(1);
      expect(session.chatMessages[0].content).toBe("Hello!");
    });
  ...
  describe("status derivation", () => {
    ...
    it("defaults to idle", () => {
      expect(store.getState().agentSessions.get("s1")!.status).toBe("idle");
    });

    it("working state", () => {
      store.getState().setSessionWorking("s1", true);
      expect(store.getState().agentSessions.get("s1")!.status).toBe("working");
    });

    it("error takes priority over working", () => {
      store.getState().setSessionWorking("s1", true);
      store.getState().setSessionError("s1", "fail");
      expect(store.getState().agentSessions.get("s1")!.status).toBe("error");
    });

    it("needs-input takes priority over working", () => {
      store.getState().setSessionWorking("s1", true);
      store.getState().setSessionNeedsInput("s1", true);
      expect(store.getState().agentSessions.get("s1")!.status).toBe("needs-input");
    });
  ...
  describe("CLI validation state", () => {
    it("starts as null (unchecked)", () => {
      expect(store.getState().cliAvailable["claude"] ?? null).toBeNull();
      expect(store.getState().cliError["claude"] ?? null).toBeNull();
    });
  });
  ...
  describe("multi-session event routing", () => {
    ...
    it("routes events to correct session by id", () => {
      ...
      expect(store.getState().agentSessions.get("s1")!.chatMessages).toHaveLength(0);
      expect(store.getState().agentSessions.get("s2")!.chatMessages).toHaveLength(1);
    });
    ...
    it("session status is independent", () => {
      ...
      expect(store.getState().agentSessions.get("s1")!.status).toBe("error");
      expect(store.getState().agentSessions.get("s2")!.status).toBe("working");
    });
  });
  ...
  describe("attention tracking", () => {
    ...
    it("switching to a session clears its attention", () => {
      ...
      store.getState().setActiveSessionId("s2");
      const session = store.getState().agentSessions.get("s2")!;
      expect(session.needsInput).toBe(false);
    });

    it("attention count across sessions", () => {
      store.getState().createSessionLocal("s2", "Session 2");
      store.getState().createSessionLocal("s3", "Session 3");
      store.getState().setSessionNeedsInput("s1", true);
  ...
  describe("rich cards", () => {
    ...
    it("upsertActivityLine creates and updates activity", () => {
      ...
      let session = store.getState().agentSessions.get("s1")!;
      expect(session.chatMessages).toHaveLength(1);
      ...
      session = store.getState().agentSessions.get("s1")!;
      expect(session.chatMessages).toHaveLength(1);
  ...
  describe("outcome state", () => {
    ...
    it("sets preview URL", () => {
      ...
      expect(store.getState().agentSessions.get("s1")!.previewUrl).toBe("http://localhost:3000");
    });

    it("sets test summary", () => {
      store.getState().setSessionTestSummary("s1", { passed: 10, failed: 2, total: 12 });
      const summary = store.getState().agentSessions.get("s1")!.testSummary;
