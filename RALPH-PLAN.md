...
- [x] **2c: Add CLI info TypeScript wrapper and display** — Add `detectAvailableClis()` wrapper in `src/lib/tauri.ts` or `src/lib/agentCommands.ts`. In `src/components/layout/TitleBar.tsx`, add a small info badge next to the existing repo/skill badges that shows the count of available CLIs with a tooltip listing them. Call `detectAvailableClis()` on app startup and cache the result in a simple React state or Zustand slice.
