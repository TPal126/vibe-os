...
// ── Adapter factory type for tests ──

/// In test mode, the factory takes (backend_name, spawn_args) and returns a session_id.
/// No AppHandle needed since tests verify DB state, not event delivery.
#[cfg(any(test, feature = "test-fake"))]
pub type TestAdapterFactory =
    Box<dyn Fn(&str, &SpawnArgs) -> Result<String, String> + Send + Sync>;

// ── WorkflowEngine — testable core ──

/// Testable core engine. Takes `&Connection` directly — no Tauri state resolution.
/// In production, `app` is `Some(handle)` for adapter spawning and event emission.
/// In tests, `app` is `None` and `test_factory` provides adapter dispatch.
pub struct WorkflowEngine<'a> {
    conn: &'a Connection,
    app: Option<AppHandle>,
    #[cfg(any(test, feature = "test-fake"))]
    test_factory: Option<TestAdapterFactory>,
}

impl<'a> WorkflowEngine<'a> {
    /// Production constructor — uses real adapters via AppHandle.
    pub fn new(conn: &'a Connection, app: AppHandle) -> Self {
        Self {
            conn,
            app: Some(app),
            #[cfg(any(test, feature = "test-fake"))]
    ...
    /// Test constructor — injectable adapter factory, no AppHandle needed.
    #[cfg(any(test, feature = "test-fake"))]
    pub fn for_test(conn: &'a Connection, factory: TestAdapterFactory) -> Self {
        Self {
            conn,
            app: None,
    ...
    /// In test mode with a factory, uses the factory (no real process spawning).
    /// In production, uses real adapters via AppHandle.
    fn dispatch_adapter(&self, backend: &str, args: SpawnArgs) -> Result<String, String> {
        #[cfg(any(test, feature = "test-fake"))]
        if let Some(ref factory) = self.test_factory {
            return factory(backend, &args);
        }

        let app = self.app.as_ref().ok_or("No AppHandle available")?;
        match backend {
            "claude" => {
                let adapter = ClaudeAdapter;
                adapter.spawn(args, app)
            }
            "codex" => {
    ...
    pub fn start_pipeline(&self, pipeline_id: &str) -> Result<String, String> {
    ...
    fn start_phase(
        ...
        let workspace_path: String = self
            .conn
            .query_row(
        ...
        let spawn_result = self.dispatch_adapter(&backend, args);
    ...
    pub fn on_phase_complete(
        ...
        let (phase_id, session_id): (String, String) = self
            .conn
            .query_row(
        ...
        let (gate_after, position, pipeline_id, label): (String, i64, String, String) = self
            .conn
            .query_row(
        ...
        let next_phase_id: Option<String> = self
            .conn
            .query_row(
        ...
        let (summary, artifact_path): (Option<String>, Option<String>) = self
            .conn
            .query_row(
        ...
        match (gate_after.as_str(), next_phase_id) {
            ("auto", Some(next_id)) => {
                ...
                let context =
                    build_handoff_context(&label, summary.as_deref(), artifact_path.as_deref());
            ...
            ("gated", Some(next_id)) | (_, Some(next_id)) => {
                ...
                // Emit PhaseTransition event (only in production with AppHandle)
    ...
    pub fn advance_gate(&self, pipeline_run_id: &str) -> Result<(), String> {
        ...
        let (position, pipeline_id, label): (i64, String, String) = self
            .conn
            .query_row(
...
// ── WorkflowRunner — thin Tauri layer ──
...
impl WorkflowRunner {
    ...
    pub async fn start_pipeline(&self, pipeline_id: &str) -> Result<String, String> {
        ...
        let engine = WorkflowEngine::new(&conn, self.app.clone());
    ...
    pub async fn on_phase_complete(
        ...
        let conn = db.lock().map_err(|e| format!("DB lock: {}", e))?;
        let engine = WorkflowEngine::new(&conn, self.app.clone());
        engine.on_phase_complete(pipeline_run_id, phase_run_id)
    }
    ...
    pub async fn advance_gate(&self, pipeline_run_id: &str) -> Result<(), String> {
        ...
        let conn = db.lock().map_err(|e| format!("DB lock: {}", e))?;
        let engine = WorkflowEngine::new(&conn, self.app.clone());
        engine.advance_gate(pipeline_run_id)
    }
    ...
    pub async fn get_run_status(
        ...
        let conn = db.lock().map_err(|e| format!("DB lock: {}", e))?;
        let engine = WorkflowEngine::new(&conn, self.app.clone());
        engine.get_run_status(pipeline_run_id)
    }
}
